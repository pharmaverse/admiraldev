...
condition being tested. Use one of the following instead:
...
- `expect_snapshot(..., error = TRUE, cnd_class = TRUE)` (drop `error = TRUE`
  when testing a warning), which records the condition class together with
  the full message text in a snapshot file. This is the preferred approach in
  `{admiral}`, as it also locks in the user-facing message text: any
  unintentional change to the wording will cause the snapshot test to fail
  and be reviewed.
...
Both `expect_error()` calls confirm the condition class `"assert_logical_scalar"`
is raised, and `expect_snapshot()` additionally locks in the exact message
text. If `assert_logical_scalar()` were changed to throw an unrelated error,
or stopped throwing an error at all, these tests would correctly fail.
