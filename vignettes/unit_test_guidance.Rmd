...
use onr the of the following two options instead:

- **If it is defined in the tested function**: Use the `class` argument of 
 `expect_error()`/`expect_warning()`, when the
...
- **Otherwise**: Use `expect_snapshot(..., error = TRUE, cnd_class = TRUE)` 
