...
as hotfixes. At the bottom, you can explore the status of any admiral/pharmaverse-related
uploads to CRAN.
...
The table below shows the CRAN approval status for any recently-submitted packages with "pharmaverse" or "admiral" within their name. It refreshes every hour.
...
cran_incoming_admiral_pharmaverse <- cran_incoming %>% 
  filter(grepl("pharmaverse|admiral", package))
```
...
if (nrow(cran_incoming_admiral_pharmaverse > 0)) {
  
  cran_incoming_admiral_pharmaverse %>% 
  ...
  c("No package submissions currently awaiting CRAN approval!")
}
