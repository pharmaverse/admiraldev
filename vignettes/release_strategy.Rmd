...
1) Bundle up (from `main`) and send off to CRAN. See the [chapter](https://r-pkgs.org/release.html#decide-the-release-type) in R Packages for more details.
...
GitHub to "release" the package onto GitHub.  This releases onto Github archives
the version of code within the `main` branch, attaches the News/Changelog file,
bundles the code into a `tar.gz` file and makes a validation report via the
GitHub action `validation` from
[insightsengineering/validatoR](https://github.com/insightsengineering/thevalidatoR).
Please see past [admiral
releases](https://github.com/pharmaverse/admiral/releases) for reference and the
...
1) Create the next development version:
    a) Create a new feature branch from `main`.
    a) Call `usethis::create_dev_version()` to update `NEWS.md` (adds a new
...
created from the most recent release of the package. There is no need to use
...
1) If all PRs for the hot fix are merged (to `patch`), bundle up (from `patch`) and send off to CRAN. See the [chapter](https://r-pkgs.org/release.html#decide-the-release-type) in R Packages for more details.
...
on GitHub to "release" the package onto GitHub and select the `patch` branch as
target.  This releases onto Github archives the version of code within the
`patch` branch, attaches the News/Changelog file, bundles the code into a
`tar.gz` file and makes a validation report via the GitHub action `validation`
from
[insightsengineering/validatoR](https://github.com/insightsengineering/thevalidatoR).
Please see past [admiral
releases](https://github.com/pharmaverse/admiral/releases) for reference and the
