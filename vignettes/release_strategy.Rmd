...
1) Create the release version:
    ...
    1) Update the `Versions:` field in the `DESCRIPTION` file.
    1) Update `NEWS.md` (replace "development version" with the version to be
    released)
    1) Create a PR into `main`. The title of the PR must contain "[skip vbump]".
...
1) Create the next development version:
    1) Create a new feature branch from `main`.
    1) Update the `Versions:` field in the `DESCRIPTION` file.
    1) Update `NEWS.md` (add a new section "# admiral ("development version")")
    1) Create a PR into `main`. Merging the PR will update the version in the
