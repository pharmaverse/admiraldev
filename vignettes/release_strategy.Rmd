...
1) Create the release version:
    ...
    a) Update the `Versions:` field in the `DESCRIPTION` file.
    a) Update `NEWS.md` (replace "(development version)" with the version to be
    released)
...
1) Create the next development version:
    ...
    a) Create a PR into `main` and merge it. Merging the PR will update the
    version in the `DESCRIPTION` file to a development version.
