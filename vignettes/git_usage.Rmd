...
The name of the branch must be prefixed with the issue number, followed by a short but meaningful description and the `@<target_branch>` suffix, if it requires building off a branch of an upstream repo. The `@<target_branch>` suffix is used in our CI/CD pipelines, e.g. when running `R CMD check`. It ensures that `{admiral}`'s dependencies such as `{pharmaversesdtm}` and `{admiraldev}` are installed from the specified target branch. So when the target branch is set to `@main` the dependencies will be installed from those package's respective `main` branches rather than installing the latest development version. This ensures that we test `{admiral}` against the corresponding versions of its dependencies. That way all packages are kept in sync.
