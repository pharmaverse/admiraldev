...
* We encourage the use of tidyverse (e.g. dplyr) over similar functionality existing in base R
...
* Each function that is used to derive one or multiple variable(s) or parameter(s) is required to be unit tested.
...
Arguments which only expect one value or variable name must be a singular version of the word(s), e.g., `missing_value` or `new_var`.

Arguments which expect several values or variable names (as a list, expressions, etc.) must be a plural version of the word(s), e.g., `missing_values` or `new_vars`.
...
We could be a lot better at doing this
...
#' 
#'   Should we update this to show our standardization of roxygen text?
#'
...
Mention standardization of roxygen label/description here?
...
in the description. (Do we actually do this?)
...
Are those last few still even applicable? 
...
This is more of a R in general type of thing, less so admiral.
...
Recommend we remove or write an update to what it actually is.
