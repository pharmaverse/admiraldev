...
permitted values the `@permitted` tag should be used (see `rdx_roclet()` for
more details).
...
* `@examples` or `@caption`, `@info`, `@code`: Fully self-contained examples of how to use the function.
Self-contained means that, if this code is executed in a new R session, it will
run without errors. That means any packages need to be loaded with `library()`
and any datasets needed either to be created directly inside the example code or
loaded using `data()`. If a dataset is created in the example, it should be done
so using the function `tribble()` (specify `library(dplyr)` before calling this
function). If other functions are called in the example, please specify
...
Guidance](unit_test_guidance.html#set-up-the-test-script). Make sure to align
columns as this ensures quick code readability.

    The `@examples` tag should be used for simple functions which requires only
    a few examples and no explanation. For more complex functions, the
    `@caption`, `@info`, and `@code` tags should be used. The `@caption` tag
    should be used to provide a brief description of the example. The `@info`
    tag should be used to provide a more detailed description of the example.
    The `@code` tag should be used to provide the code for the example. See
