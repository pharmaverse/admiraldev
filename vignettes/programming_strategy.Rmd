...
`.lintr` configuration files use the `admiral_linters()` function. This contains 
...
(so as not to expose it to users) and can be loaded within the `.lintr` configuration 
file with `source(system.file("inst/lintr/linters.R", package = "admiraldev")`. 
