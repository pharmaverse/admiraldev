...
test_that("assert_named_exprs Test 52: error if `arg` is not a named list of expressions (deprecation warn)", { # nolint
  ...
  expect_warning(
    try(example_fun(list(1, 2, TRUE)), silent = TRUE),
    class = "lifecycle_warning_deprecated"
  )
  expect_warning(
    try(example_fun(arg), silent = TRUE),
    class = "lifecycle_warning_deprecated"
  )
})
...
test_that("assert_named_exprs Test 53: no error if `arg` is NULL and optional is TRUE (deprecation warn)", { # nolint
  ...
  expect_warning(
    try(example_fun(NULL), silent = TRUE),
    class = "lifecycle_warning_deprecated"
  )
})
...
test_that("assert_function Test 59: If dot-dot-dot is a argument", {
  example_fun <- function(arg) {
    assert_function(arg, params = c("..."))
  }
  expect_invisible(example_fun(mutate))
})


# assert_function_param ----
## Test 60: no error if `arg` is a parameter of a function (deprecation warn) ----
test_that("assert_function_param Test 60: no error if `arg` is a parameter of a function (deprecation warn)", { # nolint
  hello <- function(name) {
    print(sprintf("Hello %s", name))
  }

  expect_warning(
    try(assert_function_param("hello", params = "name"), silent = TRUE),
    class = "lifecycle_warning_deprecated"
  )
})

## Test 61: error if expected function parameters are missing (deprecation warn) ----
test_that("assert_function_param Test 61: error if expected function parameters are missing (deprecation warn)", { # nolint
  hello <- function(name) {
    print(sprintf("Hello %s", name))
  }
