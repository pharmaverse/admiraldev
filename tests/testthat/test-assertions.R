...
## Test 38: error if length is not as expected ----
test_that("assert_numeric_vector Test 38: error if length is not as expected", {
  numbers <- c(1, 2, 3)
  expect_snapshot(
    assert_numeric_vector(
      numbers,
...
# assert_function ----
## Test 52: error if `arg` is not a function ----
test_that("assert_function Test 52: error if `arg` is not a function", {
  example_fun <- function(arg) {
    assert_function(arg)
  }
...
test_that("assert_function Test 53: no error if `arg` is NULL and optional is TRUE", {
  example_fun <- function(arg) {
...
## Test 54: no error if `arg` is a function with all parameters defined ----
test_that("assert_function Test 54: no error if `arg` is a function with all parameters defined", {
  example_fun <- function(arg) {
...
## Test 55: error if  `params`  is missing with no default ----
test_that("assert_function Test 55: error if  `params`  is missing with no default", {
  example_fun <- function(arg) {
    assert_function(arg, params = c("x"))
  }
...
## Test 56: If dot-dot-dot is a argument ----
test_that("assert_function Test 56: If dot-dot-dot is a argument", {
...
# assert_unit ----
## Test 57: no error if the parameter is provided in the expected unit ----
test_that("assert_unit Test 57: no error if the parameter is provided in the expected unit", {
  advs <- dplyr::tribble(
...
## Test 58: no error for multiple expected units ----
test_that("assert_unit Test 58: no error for multiple expected units", {
  advs <- dplyr::tribble(
    ~USUBJID, ~VSTESTCD, ~VSTRESN, ~VSSTRESU, ~PARAMCD, ~AVAL,
    "P01",    "WEIGHT",      80.1, "kg",      "WEIGHT",  80.1,
    "P02",    "WEIGHT",      85.7, "kg",      "WEIGHT",  85.7
...
## Test 59: no error if all units NA ----
test_that("assert_unit Test 59: no error if all units NA", {
  advs <- dplyr::tribble(
    ~USUBJID, ~VSTESTCD, ~VSTRESN, ~VSSTRESU,     ~PARAMCD, ~AVAL,
...
## Test 60: error if multiple units in the input dataset ----
test_that("assert_unit Test 60: error if multiple units in the input dataset", {
  advs <- dplyr::tribble(
    ~USUBJID, ~VSTESTCD, ~VSTRESN, ~VSSTRESU, ~PARAMCD, ~AVAL,
    "P01",    "WEIGHT",      80.1, "kg",      "WEIGHT",  80.1,
...
## Test 61: error if unexpected unit in the input dataset ----
test_that("assert_unit Test 61: error if unexpected unit in the input dataset", {
  advs <- dplyr::tribble(
    ~USUBJID, ~VSTESTCD, ~VSTRESN, ~VSSTRESU, ~PARAMCD, ~AVAL,
    "P01",    "WEIGHT",      80.1, "kg",      "WEIGHT",  80.1,
    "P02",    "WEIGHT",      85.7, "kg",      "WEIGHT",  85.7
...
## Test 62: error if get_unit_expr invalid ----
test_that("assert_unit Test 62: error if get_unit_expr invalid", {
  advs <- dplyr::tribble(
    ~USUBJID, ~VSTESTCD, ~VSTRESN, ~VSSTRESU, ~PARAMCD, ~AVAL,
    "P01",    "WEIGHT",      80.1, "kg",      "WEIGHT",  80.1,
    "P02",    "WEIGHT",      85.7, "kg",      "WEIGHT",  85.7
...
## Test 82: dataset is returned invisible if one-to-one ----
test_that("assert_one_to_one Test 82: dataset is returned invisible if one-to-one", {
  df <- dplyr::tribble(
    ~SPECIES, ~SPECIESN,
    "DOG",           1L,
    "CAT",           2L,
